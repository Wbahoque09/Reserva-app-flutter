...
class BooksLoans extends StatelessWidget {
  const BooksLoans({
    super.key,
    required this.size,
  });
              ...
              if (!snapshot.hasData) {
                return Column(
                  ...
                  children: [
                    ...
                    const Text('No hay libros por prestados'),
                  ],
                );
              }
              final dataReserve = snapshot.data;
              final bookFilter = data
                  .where((book) => dataReserve!.idLoans.contains(book.id))
                  .toList();
...
Stream<LoansModel> returReserve() {
